...
{
    {
        ...
        REQUIRE(interp.parse(iss));
        REQUIRE(interp.eval() == Expression(std::make_tuple(0., 0.), std::make_tuple(100., 0.), atan2(0, -1)));
    }
  ...
  }
  {
    ...
    REQUIRE(interp.parse(iss));
    REQUIRE(interp.eval() == Expression(std::make_tuple(0., 0.), std::make_tuple(10., 0.)));
  }
  {
    ...
    REQUIRE(interp.parse(iss));
    interp.eval();
    REQUIRE(interp.eval() == Expression(std::make_tuple(0., 0.), std::make_tuple(10., 0.), atan2(0, -1)));
  }
}
...
TEST_CASE("Test file test_car.slp", "[interpreter]")
{
    std::string fname = TEST_FILE_DIR + "/test_car.slp";
